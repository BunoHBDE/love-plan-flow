-- =====================================================================
-- INDICADOR DE AUTORIA POR CAMPO (substitui a tela de Revisão IA)
-- =====================================================================
-- A tela de Revisão IA nunca foi usada e não vai ser. No lugar dela, o
-- lead ganha um mapa `campos_ia` (jsonb) com os campos que hoje têm o
-- valor que a IA escreveu por último, sem edição humana depois. A gaveta
-- do lead usa isso para mostrar um símbolo de IA ao lado de cada campo.
--
-- Campos rastreados (os mesmos que aplicar_sugestoes_ia escreve e que
-- aparecem como campo editável na gaveta): 'nome', 'data', 'convidados',
-- 'observacoes'. Etapa fica de fora: não é um campo com valor único que
-- se sobrescreve, é uma posição, e já tem histórico próprio ("ver etapas
-- anteriores").
--
-- Regra: aplicar_sugestoes_ia() ACRESCENTA a chave quando escreve o
-- campo. Qualquer edição humana (atualizarLead / atualizarContato no
-- frontend) REMOVE a chave correspondente — silenciosamente, sem deixar
-- marca, exatamente como pedido. reverter_evento_ia() também remove a
-- chave dos campos desfeitos, porque depois do desfazer o valor atual
-- não é mais o que a IA escreveu.
-- =====================================================================

ALTER TABLE public.crm_leads
  ADD COLUMN IF NOT EXISTS campos_ia JSONB NOT NULL DEFAULT '{}'::jsonb;

-- Chamada pelo frontend logo após uma edição humana bem-sucedida, para
-- tirar o símbolo de IA dos campos que acabaram de ser sobrescritos à
-- mão. SECURITY INVOKER: passa pela RLS normal de crm_leads.
CREATE OR REPLACE FUNCTION public.crm_leads_limpar_campos_ia(
  p_lead_id uuid,
  p_campos  text[]
)
RETURNS void
...
AS $$
  UPDATE public.crm_leads
     SET campos_ia = campos_ia - p_campos
   WHERE id = p_lead_id;
$$;

REVOKE ALL ON FUNCTION public.crm_leads_limpar_campos_ia(uuid, text[]) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.crm_leads_limpar_campos_ia(uuid, text[]) TO authenticated;


-- ---------------------------------------------------------------------
-- aplicar_sugestoes_ia(): acrescenta a campos_ia sempre que grava nome,
-- data, convidados ou observacoes.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- reverter_evento_ia(): tira dos campos_ia o que foi desfeito.
-- ---------------------------------------------------------------------
...
BEGIN
  ...
  v_campos_remover := ARRAY(
    SELECT x FROM unnest(ARRAY['nome','data','convidados','observacoes']) x
     WHERE antes ? (CASE WHEN x = 'data' THEN 'data_evento' ELSE x END)
  );
  ...
  UPDATE crm_leads SET
    ...
    campos_ia          = campos_ia - v_campos_remover
  WHERE id = ev.lead_id
