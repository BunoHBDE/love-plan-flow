-- =====================================================================
-- A IA PASSA A GRAVAR NOME, CIDADE E SUA PRÓPRIA ANÁLISE (observacoes)
-- =====================================================================
-- Diagnóstico: nome_extraido e cidade_extraida sempre foram calculados
-- pela classificação (ia_sugestoes) mas nunca chegavam ao CRM —
-- aplicar_sugestoes_ia() nunca tinha código para gravá-los. Confirmado
-- em produção: 28 leads com nome extraído e cliente ainda com nome
-- numérico (telefone), 81 leads com cidade extraída e cidade vazia.
--
-- NOME mora em `clients.nome` (crm_leads não tem coluna própria), então
-- esta função passa a tocar duas tabelas. Trava de segurança: só
-- sobrescreve um nome que hoje é "auto-gerado" (só dígitos — é o mesmo
-- padrão que vincular_mensagens_a_leads() já usa pra detectar isso).
-- Nome já editado por humano nunca é tocado.
--
-- CIDADE volta a ser gravada com a mesma regra de sempre: ausência
-- preserva o valor atual.
--
-- OBSERVACOES muda de função: nunca teve um vendedor usando (0 de 568
-- leads no banco antes desta migração), então vira a nota de análise da
-- IA. A cada nova classificação o texto é SUBSTITUÍDO (não some: o texto
-- anterior fica em crm_lead_events.meta->'antes'->'observacoes',
-- restaurável por reverter_evento_ia).
--
-- Autoria: toda escrita feita por esta função carimba
-- atualizado_por_tipo='ia' / atualizado_por_id=ia_autor_uuid() — o
-- trigger de 20260918012118 deixa passar porque reconhece a assinatura.
-- =====================================================================
...
BEGIN
  ...
  LOOP
    ...
    -- Só sobrescreve um nome que hoje é "auto-gerado" (só dígitos: o
    -- placeholder que vincular_mensagens_a_leads() usa quando não tem nome
    -- de perfil do WhatsApp). Nome que já é texto — mesmo que informal —
    -- pode ter sido corrigido por um humano, então a IA nunca mexe nele.
    n_nome := NULL;
    ...
    -- ---- ETAPA e encerramento (inalterado nesta migração) -----------------
...
-- ---------------------------------------------------------------------
-- Reversão: agora também restaura nome (em clients) e observacoes
-- ---------------------------------------------------------------------
